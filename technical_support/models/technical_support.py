...
class technical_support_order(models.Model):
    ...
    date_close = fields.Datetime('Close Date', required=True, states={'done':[('readonly',True)],'cancel':[('readonly',True)]}, default=time.strftime('%Y-%m-%d %H:%M:%S'))
...
class technical_support_request(models.Model):
    ...
    close_date = fields.Datetime('Close Date', required=True, readonly=True, states={'draft':[('readonly',False)],'claim':[('readonly',False)]}, default=time.strftime('%Y-%m-%d %H:%M:%S'))
